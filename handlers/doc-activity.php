...
$tracks = $con->getData("SELECT track_date, document_status, track_option, (SELECT CONCAT(users.fname, ' ', users.lname) FROM users WHERE users.id = tracks.document_status_user) staff, (SELECT offices.office FROM offices WHERE id = tracks.track_office) office FROM tracks WHERE document_id = ".$_POST['id']);
